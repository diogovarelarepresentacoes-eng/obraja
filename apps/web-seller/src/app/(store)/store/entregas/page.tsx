...
const fleetStatus: Record<string, { label: string; dot: string; textColor: string; rowBg: string }> = {
  'Em rota': { label: 'Em rota', dot: '🟠', textColor: 'text-orange-600', rowBg: 'bg-orange-50' },
  Disponível: { label: 'Disponível', dot: '🟢', textColor: 'text-green-600', rowBg: '' },
  Folga: { label: 'Folga', dot: '🔴', textColor: 'text-red-500', rowBg: 'bg-red-50' },
}
...
const fleet = [
  ...
  { name: 'Paulo Costa', vehicle: 'Honda Pop', type: 'Moto', plate: 'JKL-3456', status: 'Disponível', deliveries: 2, rating: '4.9', action: 'Escalar', actionStyle: 'bg-[#F1591D] text-white hover:bg-orange-600' },
  { name: 'André Silva', vehicle: 'Mercedes Sprinter', type: 'Van', plate: 'MNO-7890', status: 'Disponível', deliveries: 1, rating: '4.8', action: 'Escalar', actionStyle: 'bg-[#F1591D] text-white hover:bg-orange-600' },
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex gap-1">
          <button className="px-4 py-2 bg-[#F1591D] text-white text-sm font-bold rounded-lg">
            Entregas de Hoje
          </button>
      ...
      <div className="bg-white rounded-[10px] border border-[#E5E5E5] overflow-hidden">
        ...
        <div className="bg-[#1A1A1A] h-[260px] flex flex-col items-center justify-center gap-3 relative">
          <div className="flex items-center gap-3">
            <span className="text-[#F1591D] text-3xl">📍</span>
          ...
          {/* Simulated dots */}
          <div className="absolute top-12 left-1/4 w-3 h-3 rounded-full bg-[#F1591D] animate-pulse shadow-lg shadow-orange-500/50" />
          <div className="absolute top-20 left-1/2 w-3 h-3 rounded-full bg-[#F1591D] animate-pulse shadow-lg shadow-orange-500/50" style={{ animationDelay: '0.5s' }} />
          <div className="absolute bottom-16 right-1/3 w-3 h-3 rounded-full bg-[#F1591D] animate-pulse shadow-lg shadow-orange-500/50" style={{ animationDelay: '1s' }} />
        </div>
          ...
          {activeDeliveries.map((d) => (
            <div key={d.id} className="bg-[#F8F8F8] rounded-lg border border-[#E5E5E5] p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex items-start gap-3">
                <div className="w-9 h-9 rounded-full bg-[#F1591D] flex items-center justify-center text-white font-bold text-xs shrink-0">
                  {d.vehicle === 'Moto' ? '🛵' : '🚐'}
                </div>
                <div>
                  ...
                  <p className="text-xs text-[#9E9E9E] mt-0.5">
                    Entregador: <span className="font-semibold text-[#1A1A1A]">{d.deliverer}</span> ({d.vehicle}) — Saiu: {d.departed} — Est: <span className="font-semibold text-[#F1591D]">{d.eta}</span>
                  </p>
                </div>
              </div>
                ...
                return (
                  <tr key={f.name} className={`hover:bg-[#FAFAFA] transition-colors ${st.rowBg}`}>
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-sm font-semibold ${st.textColor}`}>{st.dot} {st.label}</span>
                    </td>
